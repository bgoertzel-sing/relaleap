...
def run_phase0_smoke(config: dict[str, Any]) -> Phase0Result:
    ...
    if residual_objective not in {
        ...
        raise ValueError(
            ...
            "supervised_ce_focal, supervised_ce_temporal_consistency, "
...
def _residual_loss(
    ...
    *,
    ...
    temporal_consistency_weight: float = 0.01,
) -> Any:
    import torch
    import torch.nn.functional as F
    ...
    if objective == "supervised_ce_focal":
        ...
        target_probs = torch.exp(-per_token_ce)
        return ((1.0 - target_probs).pow(focal_gamma) * per_token_ce).mean()
    if objective == "supervised_ce_temporal_consistency":
        if prediction_logits.shape[1] < 2:
            return ce_loss
        teacher_probs = torch.softmax(prediction_logits[:, :-1, :].detach(), dim=-1)
        student_log_probs = torch.log_softmax(prediction_logits[:, 1:, :], dim=-1)
        temporal_loss = -(teacher_probs * student_log_probs).sum(dim=-1).mean()
        return ce_loss + temporal_consistency_weight * temporal_loss
