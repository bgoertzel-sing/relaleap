...
from relaleap.experiments.promoted_topk2_low_rank_value_gate import (
    INSUFFICIENT_EVIDENCE,
    LOW_RANK_VALUE_CANDIDATE_FOUND,
    LOW_RANK_VALUE_NOT_ESTABLISHED,
    run_promoted_topk2_low_rank_value_gate,
)


class PromotedTopk2LowRankValueGateTest(unittest.TestCase):
    def test_runs_low_rank_microtest_and_writes_gate_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config_path.write_text(
                """
run:
  experiment_id: test_low_rank_value_gate
  seed: 1
  max_steps: 2
...
""".strip()
            ...
            self.assertEqual(summary["metrics"]["low_rank_count"], 2)
            self.assertEqual(len(summary["low_rank_rows"]), 2)
            self.assertTrue((root / "gate" / "summary.json").is_file())
            self.assertTrue((root / "gate" / "low_rank_rows.csv").is_file())
    ...
    def test_finds_candidate_when_mocked_low_rank_variant_passes_gate(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            with patch(
                "relaleap.experiments.promoted_topk2_low_rank_value_gate.run_retention_churn_microtest"
            ) as mocked:
            ...
            self.assertEqual(summary["status"], "pass")
            self.assertEqual(summary["decision"], LOW_RANK_VALUE_CANDIDATE_FOUND)
            self.assertEqual(
                summary["low_rank_rows"][0][
...
def _variant(
    ...
    *,
    ...
    return {
        ...
        "value_gradient_low_rank": low_rank,
    }
