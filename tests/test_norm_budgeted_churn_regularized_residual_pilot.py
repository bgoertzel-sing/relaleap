...
class NormBudgetedChurnRegularizedResidualPilotTest(unittest.TestCase):
    ...
    def test_runs_bounded_local_pilot_from_design(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(all("churn_anchor_kl_weight" in row for row in rows))
            self.assertTrue(all("churn_flip_margin_weight" in row for row in rows))
            self.assertTrue(all("churn_logit_mse_weight" in row for row in rows))
            self.assertTrue(all("train_off_target_anchor_kl_trajectory" in row for row in rows))
            self.assertTrue(all("train_flip_margin_penalty_trajectory" in row for row in rows))
            self.assertTrue(all("final_train_off_target_anchor_kl_penalty" in row for row in rows))
            self.assertTrue(all("final_train_flip_margin_penalty" in row for row in rows))
            ...
            self.assertGreater(float(sparse["churn_anchor_kl_weight"]), 0.0)
            self.assertGreater(float(sparse["churn_flip_margin_weight"]), 0.0)
            self.assertTrue(sparse["train_off_target_anchor_kl_trajectory"])
            self.assertTrue(sparse["train_flip_margin_penalty_trajectory"])
            self.assertGreaterEqual(float(sparse["final_train_off_target_anchor_kl_penalty"]), 0.0)
            self.assertGreaterEqual(float(sparse["final_train_flip_margin_penalty"]), 0.0)
