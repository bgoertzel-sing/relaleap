from __future__ import annotations

import json
import tempfile
import unittest
from pathlib import Path

from relaleap.experiments.promoted_topk2_post_value_router_mitigation_closeout import (
    INSUFFICIENT_EVIDENCE,
    MITIGATION_CLOSEOUT_NO_PROMOTION,
    run_promoted_topk2_post_value_router_mitigation_closeout,
)


class PromotedTopk2PostValueRouterMitigationCloseoutTest(unittest.TestCase):
    def test_closes_failed_mitigation_branches_without_promotion(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            paths = _write_sources(root)
            review = root / "latest-review.md"
            review.write_text(
                "\n".join(
                    [
                        "strategic_change_level: minor",
                        "notify_ben: false",
                        "recommended_next_action: Run the planned router-policy mitigation probe",
                    ]
                )
                + "\n",
                encoding="utf-8",
            )

            summary = run_promoted_topk2_post_value_router_mitigation_closeout(
                out_dir=root / "closeout",
                router_policy_probe_path=paths["router"],
                branch_selector_path=paths["branch"],
                order_averaging_probe_path=paths["order"],
                update_decomposition_audit_path=paths["decomposition"],
                value_mitigation_gate_path=paths["value"],
                low_rank_value_gate_path=paths["low_rank"],
                commutator_value_penalty_probe_path=paths["penalty"],
                finite_update_report_path=paths["finite"],
                strategy_review_path=review,
            )

            self.assertEqual(summary["status"], "pass")
            self.assertEqual(summary["decision"], MITIGATION_CLOSEOUT_NO_PROMOTION)
            self.assertEqual(
                summary["selected_next_action"],
                "pairwise_value_interaction_localization_audit",
            )
            self.assertIsNone(summary["next_command"])
            self.assertEqual(
                summary["claim_statuses"]["order_averaging"],
                "diagnostic_only_not_promoted",
            )
            self.assertEqual(
                summary["claim_statuses"]["topk2_causal_cooperation"],
                "not_supported",
            )
            self.assertEqual(
                summary["evidence"]["router_policy_reduction_fraction"],
                0.18,
            )
            self.assertFalse(summary["strategy_review"]["ben_notification_required"])
            self.assertEqual(len(summary["closeout_rows"]), 6)
            self.assertTrue((root / "closeout" / "summary.json").is_file())
            self.assertTrue((root / "closeout" / "source_rows.csv").is_file())
            self.assertTrue((root / "closeout" / "closeout_rows.csv").is_file())
            self.assertTrue((root / "closeout" / "notes.md").is_file())

    def test_fails_closed_when_router_policy_source_missing(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            paths = _write_sources(root)
            paths["router"].unlink()

            summary = run_promoted_topk2_post_value_router_mitigation_closeout(
                out_dir=root / "closeout",
                router_policy_probe_path=paths["router"],
                branch_selector_path=paths["branch"],
                order_averaging_probe_path=paths["order"],
                update_decomposition_audit_path=paths["decomposition"],
                value_mitigation_gate_path=paths["value"],
                low_rank_value_gate_path=paths["low_rank"],
                commutator_value_penalty_probe_path=paths["penalty"],
                finite_update_report_path=paths["finite"],
                strategy_review_path=root / "missing-review.md",
            )

            self.assertEqual(summary["status"], "fail")
            self.assertEqual(summary["decision"], INSUFFICIENT_EVIDENCE)
            fields = {
                (failure.get("source"), failure.get("field"))
                for failure in summary["failures"]
            }
            self.assertIn(("router_policy_probe", "source_artifact"), fields)


def _write_sources(root: Path) -> dict[str, Path]:
    paths = {
        "router": root / "router.json",
        "branch": root / "branch.json",
        "order": root / "order.json",
        "decomposition": root / "decomposition.json",
        "value": root / "value.json",
        "low_rank": root / "low_rank.json",
        "penalty": root / "penalty.json",
        "finite": root / "finite.json",
    }
    _write_json(
        paths["router"],
        {
            "status": "pass",
            "decision": "value_composition_prioritized_over_router_policy",
            "router_policy_rows": [
                {
                    "variant": "dynamic_contextual_topk2",
                    "commutator_anchor_logit_mse_reduction_fraction": 0.0,
                },
                {
                    "variant": "router_frozen_transfer_topk2",
                    "commutator_anchor_logit_mse_reduction_fraction": -0.014,
                },
                {
                    "variant": "pinned_forward_final_support_topk2",
                    "commutator_anchor_logit_mse_reduction_fraction": 0.18,
                }
            ],
        },
    )
    _write_json(
        paths["branch"],
        {
            "status": "pass",
            "decision": "promoted_topk2_mitigation_branch_selected",
            "selected_next_action": "explicit_order_averaging_mitigation_probe",
        },
    )
    _write_json(
        paths["order"],
        {
            "status": "pass",
            "decision": "explicit_order_averaging_diagnostic_candidate_not_promoted",
        },
    )
    _write_json(
        paths["decomposition"],
        {
            "status": "pass",
            "decision": "value_update_dominated_order_sensitivity",
            "metrics": {
                "value_only_fraction_of_full": 1.25,
                "router_only_fraction_of_full": 0.23,
            },
        },
    )
    _write_json(
        paths["value"],
        {
            "status": "pass",
            "decision": "value_mitigation_not_established",
            "metrics": {"best_value_mitigation_reduction_fraction": 0.13},
        },
    )
    _write_json(
        paths["low_rank"],
        {
            "status": "pass",
            "decision": "low_rank_value_not_established",
            "metrics": {"best_low_rank_reduction_fraction": 0.09},
        },
    )
    _write_json(
        paths["penalty"],
        {
            "status": "pass",
            "decision": "commutator_value_penalty_not_established",
            "metrics": {"best_penalty_reduction_fraction": 0.23},
        },
    )
    _write_json(
        paths["finite"],
        {
            "status": "pass",
            "decision": "finite_update_order_sensitivity_ce_bounded_but_residual_material",
            "metrics": {
                "topk2_order_averaged_to_commutator_anchor_logit_mse_ratio": 0.25,
                "topk2_mean_order_averaged_anchor_ce_delta_vs_best_order": -0.05,
                "topk2_mean_commutator_anchor_support_churn": 0.91,
            },
        },
    )
    return paths


def _write_json(path: Path, value: dict[str, object]) -> None:
    path.write_text(json.dumps(value, indent=2, sort_keys=True) + "\n", encoding="utf-8")


if __name__ == "__main__":
    unittest.main()
