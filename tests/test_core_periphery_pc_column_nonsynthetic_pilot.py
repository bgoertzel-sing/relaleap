...
class CorePeripheryPCColumnNonSyntheticPilotTest(unittest.TestCase):
    ...
    def test_runs_local_frozen_hidden_state_pilot_with_controls(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            objectives = {
                row["variant"]: row["periphery_training_objective"]
            ...
            self.assertEqual(
                ...
                "permuted_centered_hidden_residual_mse_null",
            )
            for artifact in REQUIRED_ARTIFACTS:
                self.assertTrue((root / "out" / artifact).is_file(), artifact)
            ...
            self.assertTrue(all("periphery_training_objective" in row for row in rows))
