...
from relaleap.experiments.promoted_topk2_value_mitigation_gate import (
    INSUFFICIENT_EVIDENCE,
    VALUE_MITIGATION_CANDIDATE_FOUND,
    VALUE_MITIGATION_NOT_ESTABLISHED,
    run_promoted_topk2_value_mitigation_gate,
)


class PromotedTopk2ValueMitigationGateTest(unittest.TestCase):
    def test_runs_value_microtest_and_writes_gate_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config_path.write_text(
                """
run:
  experiment_id: test_value_mitigation_gate
  seed: 1
  max_steps: 2
...
""".strip()
            ...
            self.assertEqual(summary["metrics"]["value_mitigation_count"], 2)
            self.assertEqual(len(summary["value_mitigation_rows"]), 2)
            self.assertTrue((root / "gate" / "summary.json").is_file())
            self.assertTrue((root / "gate" / "value_mitigation_rows.csv").is_file())
    ...
    def test_finds_candidate_when_mocked_value_variant_passes_gate(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            with patch(
                "relaleap.experiments.promoted_topk2_value_mitigation_gate.run_retention_churn_microtest"
            ) as mocked:
            ...
            self.assertEqual(summary["status"], "pass")
            self.assertEqual(summary["decision"], VALUE_MITIGATION_CANDIDATE_FOUND)
            self.assertEqual(
                summary["value_mitigation_rows"][0][
...
def _variant(
    ...
    *,
    ...
    transfer: float,
    used: int | str,
    value_clip: float | str = "",
    value_scale: float | str = 1.0,
) -> dict[str, object]:
    return {
        "variant": name,
        ...
        "value_gradient_clip_norm": value_clip,
        "value_update_scale": value_scale,
    }
