    ...
    def test_trains_value_capacity_norm_control_assay_and_blocks_gpu(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for row in summary["arm_metrics"]:
                ...
                self.assertIn("support_load_entropy", row)
                self.assertIn("active_rank_proxy", row)
                ...
                self.assertGreaterEqual(row["support_load_entropy"], 0.0)
                self.assertLessEqual(row["support_load_entropy"], 1.0)
                self.assertGreaterEqual(row["support_overlap_with_oracle"], 0.0)
                self.assertLessEqual(row["support_overlap_with_oracle"], 1.0)
                self.assertGreaterEqual(row["active_rank_proxy"], 0)
